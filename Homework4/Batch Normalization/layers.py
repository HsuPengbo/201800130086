...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    X_num = X_cent                                
    X_centsq = X_cent ** 2                               
    var = np.mean(X_centsq, axis=(2,3,4), keepdims=True)  
    sd = np.sqrt(var + eps)                              
    den = 1 / sd                                        
    X_hat = X_num * den                                 
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dbeta = np.sum(d_X_out, axis=(0,2,3), keepdims=True)         
    d_X_gamma = d_X_out                            
    d_X_hat = gamma * d_X_gamma        
    dgamma = np.sum(X_hat * d_X_gamma, axis=(0,2,3), keepdims=True) 
